Feature: Managing Candidate Statuses
  ...
  Scenario: Verify that the admin can upload a txt file for Hired status candidate
    Given Admin on dashboard home page
    And Admin Shortlisted candidate
    And Admin shedule Interview for Candidate
    And Admin changed candidate interview to "passed"
    And Admin changed candidate interview to "jobed Offer"
    And Admin changed candidate interview to "Hired"
    When Admin clicks on Recruitment tab
    And Admin search for candidate
    And Admin view candidate details
    And Admin enable edit candidate switch
